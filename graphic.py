...
import numpy as np
import sympy as sp
from sympy import Matrix, symbols, solve, pretty_print
...
def centro_conica(A, B, C, D, E):
    ...
    if(sol): 
        ...
        raise ValueError("Não foi possível encontrar os centros da cônica.")
...
def graph(coef_eqg: list, clasf_c : list, Q : Matrix, tipo : str):
    ...
    a = np.sqrt(max(abs(R[2]), abs(R[4])))
    b = np.sqrt(max(abs(R[3]), abs(R[4])))
    r = 1.5 * max(a, b)
    ...
    centro_r = None
    if(R[0]*R[1] !=0):
        centro_r = centro_conica(R[2], 0, R[3], 0, 0) #a*x² + b*y² + f
    elif((R[0] == 0) and (R[1] != 0)):
        centro_r = centro_conica(0, 0, R[2], R[3], 0) #a*x + b*y² + f
    elif((R[0] != 0) and (R[1] == 0)):
        centro_r = centro_conica(R[2], 0, 0, 0, R[3]) #a*x² + b*y + f

    plt.scatter(centro_r[0], centro_r[1], color='k', s=50, marker='.', label='Centro')
    plt.legend()
    ...
    plt.gca().set_aspect('equal', adjustable = 'box') #Configura a forma como os valores são exibidos visualmente
    if(R[0]*R[1] !=0):
        plt.title(f"{tipo} \n ({R[0]:5.2f})x² + ({R[1]:5.2f})y² + ({R[2]:5.2f}) = 0",
              fontdict={
                    'family': 'Times',  
                    'weight': 'bold',      
                    'size': 10           
